...
func EventlogsallHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/webhook/logs%s", cfg.BaseURL, queryString)
		...
		var result models.GetWebhookEventLogsResponse
...
func CreateEventlogsallTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_webhook_logs",
		mcp.WithDescription("List event logs"),
		...
		mcp.WithObject("filter", mcp.Description("Filter results")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    EventlogsallHandler(cfg),
	}
}
